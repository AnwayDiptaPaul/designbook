...
> [!NOTE]
> Due to the large file size of these binary assets, the `./doc-files/` folder is ignored by version control (configured in `.gitignore`). For a detailed inventory and description of all 16 regulatory PDFs and 34 design workbooks, refer to [doc-files.md](doc-files.md).
> 
> For a machine-readable (AI-ready) structured representation of the BNBC 2020 clauses, tables, and flowcharts, refer to the external [bnbc-2020-database](https://github.com/AnwayDiptaPaul/bnbc-2020-database/) repository.
...
│   │   ├── routes/
│   │   │   ├── projects.py
│   │   │   ├── revisions.py     # Endpoint for managing project snapshot revisions
│   │   │   ├── building_input.py
│   │   │   ├── loads.py
│   │   │   ├── analysis.py
│   │   │   ├── design.py
...
│   │   └── schemas/
│   │       ├── project.py
│   │       └── revision.py      # Pydantic schemas for project revisions
│   ├── domain/                  # Engineering domain primitives
│   │   ├── units.py             # Strict SI unit system boundary and Quantity dataclass
│   │   └── snapshots.py         # Canonical JSON serialization & SHA-256 snapshot hashing
│   ├── models/                  # Database ORM models
│   │   ├── base.py
│   │   ├── project.py
│   │   ├── revision.py          # ProjectRevision ORM model & RevisionStatus Enum
│   │   └── analysis.py          # AnalysisRun ORM model (linked to ProjectRevision)
│   ├── core/
│   │   ├── loads/
│   │   │   ├── dead_live.py
...
- Date, revision number (snapshots generated deterministically via SHA-256 hashes)
- Building use/occupancy (residential, commercial, industrial, mixed)
- Number of floors (basement, ground, typical, penthouse)
- Total building height
```
*Validated Limits:*
*   `num_basements`: Bounded between `0` and `20`.
*   `num_ground_floors`: Bounded between `1` and `10`.
*   `num_typical_floors`: Bounded between `0` and `200`.
*   `num_penthouse_floors`: Bounded between `0` and `20`.
*   `total_height_m`: Bounded between `0` and `1000` meters.
...
```
*Validated Limits:*
*   `latitude` & `longitude`: Must either be both defined or both null.
*   `basic_wind_speed_m_s`: Bounded between `0` and `150` m/s.
...
```
*Validated Limits:*
*   `axis`: Must be exactly `'X'` or `'Y'` (validated via regex `^[XY]$`).
*   `floor_number`: Bounded between `-20` and `300`.
*   `height_m`: Must be strictly positive and up to `100` meters.
...
### Phase 0 — Foundation Reset & Delivery Controls (Completed ✅)
- [x] Project environment setup with env safety validation
- [x] Create project capability matrix (`CAPABILITY_MATRIX.md` and endpoints)
- [x] Establish strict SI unit system boundary (`units.py`)
- [x] Implement deterministic JSON snapshotting and SHA-256 project revision hashing
- [x] Establish backend and database schema for projects & revisions

### Phase 1 — Foundation (Months 1–2 — In Progress)
- [x] Project setup, database schema, API skeleton for project revisions
- [x] Building input schemas with boundary validations (basement limits, coordinates, grid axis, floor heights)
- [ ] Building input forms (geometry, grid, materials) in frontend
...
9. **Virtual Environment & Dependencies** — The local Python environment is managed using `uv` (rather than standard pip). See [app/venv.md](app/venv.md) for environment details, a full package list, and steps to recreate or update dependencies.

10. **Immutable Revision Snapshots** — Design inputs, geometries, and loadings are versioned as immutable snapshots via SHA-256 hashes (`ProjectRevision`). Any analysis or design run must be run from a frozen snapshot to ensure calculation results remain fully reproducible.

11. **Environment Safety Gating** — The app enforces strict safety rules via `validate_environment_safety()`: in staging/production environments, `SECRET_KEY` must be configured securely, and `DEBUG` and `CREATE_SCHEMA_ON_STARTUP` options are strictly blocked. OpenSeesPy execution is gated via the `ENABLE_ANALYSIS_EXECUTION` config.

---

*End of Instructions — Version 1.2*
