...
logger = logging.getLogger("designbook.analysis")
try:
...
class OpenSeesModelBuilder:
    ...
    def __init__(self, ndm: int = 3, ndf: int = 6):
        ...
        self._transf_counter: int = 0
        self._node_map: Dict[Tuple[int, int, int], int] = {}  # (story, ix, iy) -> node_tag
    ...
    def compute_mass_from_loads(dead_load_kpa: float, trib_area_m2: float, g: float = 9.81) -> float:
        ...
        weight_kn: float = dead_load_kpa * trib_area_m2
        mass: float = weight_kn / g  # kN·s²/m
        return mass
    ...
    def build_building_model(
        ...
        col_transf_type: str = "PDelta" if use_pdelta else "Linear"
        t_beam_x: int = self._transf_counter + 1
        t_beam_y: int = self._transf_counter + 2
        t_column: int = self._transf_counter + 3
        ...
        # Generate nodes
        node_tag: int = self._node_counter + 1
        elevation: float = 0.0
        floor_elevations: List[float] = [0.0]
        ...
        for s in range(n_stories + 1):  # 0 = base, 1..n = floors
            ...
            for ix, x in enumerate(grid_x):
                for iy, y in enumerate(grid_y):
                    ...
                    cast(Dict[int, List[int]], floor_nodes)[s].append(node_tag)
            ...
            if s < n_stories:
                story_height: float = float(story_heights[s])
                elevation += story_height
        ...
        if rigid_diaphragm:
            for s in range(1, n_stories + 1):
                ...
                if len(nodes) > 1:
                    master = nodes[0]
                    slaves = cast(List[int], nodes[1:])
                    self.apply_rigid_diaphragm(master, slaves)
        
        # Define column elements
        el_tag: int = self._element_counter + 1
            ...
            for ix in range(nx):
                for iy in range(ny):
                    ...
                    if key_bot in self._node_map and key_top in self._node_map:
                        n1: int = self._node_map[key_bot]
                        n2: int = self._node_map[key_top]
        ...
        return {
            ...
            "floor_nodes": cast(Dict[int, List[int]], floor_nodes),
    ...
    def apply_winkler_foundation(self, base_nodes: List[int], spring_props: Dict[str, float]):
        ...
        for b_node in base_nodes:
            ...
            x = float(ops.nodeCoord(b_node, 1)) # type: ignore
            y = float(ops.nodeCoord(b_node, 2)) # type: ignore
            z = float(ops.nodeCoord(b_node, 3)) # type: ignore
    ...
    def extract_member_forces(self, element_tags: List[int]) -> Dict[int, Dict[str, Any]]:
        ...
        for tag in element_tags:
            try:
                raw_forces = ops.eleForce(tag)
                forces = list(raw_forces) if hasattr(raw_forces, "__iter__") else [raw_forces] # type: ignore
    ...
    def extract_story_drifts(
        ...
        for s in range(1, len(floor_elevations) + 1):
            if s not in floor_nodes:
                continue
            nodes = floor_nodes[s]
            avg_disp = sum(float(ops.nodeDisp(nd, dof)) for nd in nodes) / max(len(nodes), 1) # type: ignore
            
            story_height = floor_elevations[s - 1] - prev_elev if s > 0 else floor_elevations[0]
            inter_story_disp = avg_disp - prev_avg_disp
            drift_ratio = abs(inter_story_disp) / max(story_height, 1e-9) if story_height > 0 else 0.0
    ...
    def apply_plastic_hinges(self, element_tags: List[int], My_pos: float, My_neg: float, hinge_length_ratio: float = 0.1):
        ...
        for el_tag in element_tags:
            ...
            x = float(ops.nodeCoord(i_node, 1)) # type: ignore
            y = float(ops.nodeCoord(i_node, 2)) # type: ignore
            z = float(ops.nodeCoord(i_node, 3)) # type: ignore
            ...
            try:
                ops.remove('element', el_tag)
            except:
                pass # Already removed or mock environment
    ...
    def run_pushover_analysis(self, control_node: int, control_dof: int, d_max: float, d_incr: float) -> dict:
        ...
        for i in range(num_steps):
            ...
            ops.reactions()
            v_base = sum(float(ops.nodeReaction(n, control_dof)) for n in self._base_nodes) # type: ignore
    ...
    def run_time_history_analysis(self, dt: float, n_pts: int, accelerations: List[float], direction: int = 1, roof_node: Optional[int] = None) -> dict:
        """
        ...
        for i in range(n_pts):
            ...
            ops.reactions()
            v_base = sum(float(ops.nodeReaction(n, direction)) for n in self._base_nodes) # type: ignore
            base_shears.append(abs(v_base))
    ...
    def run_full_pipeline(
        ...
        Includes automated telemetry for performance monitoring.
        """
        t_start = time.perf_counter()
        results: Dict[str, Any] = {"gravity": None, "modal": None, "telemetry": {}}
        
        if run_gravity:
            t_grav = time.perf_counter()
            status = self.analyze_static(1)
            dt_grav = time.perf_counter() - t_grav
            results["gravity"] = {
                "status": "converged" if status == 0 else "failed",
                "code": status,
            }
            results["telemetry"]["gravity_seconds"] = round(dt_grav, 4)
            logger.info(f"Gravity analysis completed in {dt_grav:.4f}s — status: {results['gravity']['status']}")
        ...
        if run_modal and gravity_ok:
            ...
            dt_modal = time.perf_counter() - t_modal
            EPSILON = 1e-15
            results["modal"] = {
                "periods": periods,
                "frequencies": [1.0/max(t, EPSILON) if t > 0 and t < 1e10 else 0.0 for t in periods],
            }
            results["telemetry"]["modal_seconds"] = round(dt_modal, 4)
            logger.info(f"Modal analysis ({n_modes} modes) completed in {dt_modal:.4f}s")
        
        dt_total = time.perf_counter() - t_start
        results["telemetry"]["total_pipeline_seconds"] = round(dt_total, 4)
        results["telemetry"]["n_nodes"] = self._node_counter
        results["telemetry"]["n_elements"] = self._element_counter
        logger.info(f"Full pipeline completed in {dt_total:.4f}s ({self._node_counter} nodes, {self._element_counter} elements)")
    ...
    def analyze_modal(self, num_modes: int) -> List[float]:
        ...
        vals_iterable = list(vals) if hasattr(vals, "__iter__") else [vals] # type: ignore
        periods = []
        for v in vals_iterable:
            if float(v) > 0: # type: ignore
                periods.append(2 * math.pi / math.sqrt(float(v))) # type: ignore
            else:
