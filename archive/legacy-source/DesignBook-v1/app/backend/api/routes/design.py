...
async def design_all_members(
    ...
    """Design all members in the project via StructuralDesignService dispatch."""
    from backend.core.design.service import StructuralDesignService
    ...
    for m in members:
        m.status = MemberStatus.DESIGNING
        forces = m.properties.get("forces", {"Mu": 150.0, "Vu": 50.0, "Pu": 1200.0})
        inputs = {
            "fc": float(m.concrete_grade_mpa.value),
            "fy": float(m.steel_grade_mpa.value),
            "width": m.width_mm or 300.0,
            "depth": m.depth_mm or 600.0,
            "cover": m.clear_cover_mm or 40.0,
            **m.properties,
        }
        design_result = StructuralDesignService.design_member(m.member_type, inputs, forces)
        m.design_results = design_result

        all_ok = all(
            res.get("status") == "OK"
            for res in design_result.values()
            if isinstance(res, dict) and "status" in res
        )
        m.status = MemberStatus.PASS if all_ok else MemberStatus.FAIL
